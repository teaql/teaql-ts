...
test.each([[OrderBy.desc('id'), '<'], [OrderBy.asc('id'), '>']])('continuous page uses seek for %s', async (order, operator) => {
  const driver = new Driver(); const client = new Client(driver); const ctx = new UserContext(); ctx.userIdentifier = 'tenant:user';
  for (const offset of [0, 10]) {
    ...
    await client.executeQuery(ctx.prepareQuery(query));
  }
  expect(ctx.continuousPagePlan).toBe('CURSOR_SEEK');

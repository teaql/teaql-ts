...
export class QueryParser {
    static parse(queryStr: string): any {
        // Expected format: Q.tasks().withNameContaining("bug").withVersionIs(2)
        
        const entityMatch = queryStr.match(/^Q\.([a-zA-Z0-9_]+)\(\)/);
        if (!entityMatch) {
            throw new Error("Invalid query format. Must start with Q.entityName()");
        }
        
        const entityMethod = entityMatch[1];
        if (typeof (Q as any)[entityMethod] !== 'function') {
            throw new Error(`Entity method ${entityMethod} not found on Q`);
        }
        
        // Instantiate the request builder (e.g., TaskRequest)
        const requestObj = (Q as any)[entityMethod]();

...
exports.BrowserSQLiteTeaQLClient = exports.BrowserSQLiteDriver = void 0;
...
class BrowserSQLiteTransport {
    constructor(worker) {
        ...
        this.onMessage = (event) => {
            ...
            if (!message || message.type !== 'teaql-browser-sqlite-response')
                return;
    ...
    request(operation, payload) {
        if (this.closed)
            return Promise.reject(new Error('Browser SQLite worker is closed'));
...
class BrowserSQLiteDriver {
    ...
    identifier(value) { return `"${(0, core_1.assertSafeIdentifier)(value)}"`; }
    placeholder(_index) { return '?'; }
    contains(columnSql, placeholder) {
        return `CAST(${columnSql} AS TEXT) LIKE '%' || ${placeholder} || '%'`;
    }
    aggregateFunction(name) { return (0, core_1.standardAggregateFunction)(name); }
    encode(value, column) {
    ...
    async exclusive(work) {
        ...
        let release;
        this.accessTail = new Promise(resolve => { release = resolve; });
        await predecessor;
        try {
            return await work();
        }
    ...
    transaction(work) {
        return this.exclusive(async () => {
            ...
            try {
                ...
                catch { /* preserve the business error */ }
                throw error;
            }
        });
    }
    async ensureSchema(schemas) {
        for (const schema of Object.values(schemas)) {
            ...
            for (const [field, column] of Object.entries(schema.columns)) {
                if (field === 'id' || field === 'version' || existing.has(column.columnName))
                    continue;
...
exports.BrowserSQLiteDriver = BrowserSQLiteDriver;
/** TeaQL SQL client for browser-local SQLite/WASM. */
class BrowserSQLiteTeaQLClient extends core_1.AbstractSQLTeaQLClient {
...
exports.BrowserSQLiteTeaQLClient = BrowserSQLiteTeaQLClient;

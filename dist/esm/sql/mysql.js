...
var MySQLDriver = class {
  ...
  async ensureSchema(schemas) {
    ...
    for (const index of canonicalRelationIndexes(schemas)) {
      ...
      if (!existing.rowCount) {
        await this.query(
          `CREATE INDEX ${this.identifier(index.name)} ON ${this.identifier(index.table)} (${this.identifier(index.foreignColumn)}, ${this.identifier(index.idColumn)} DESC)`
        );
      }
    }
    await this.query(
...
var MySQLTeaQLClient = class extends AbstractSQLTeaQLClient {
